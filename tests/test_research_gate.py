...
def test_selection_uses_train_pnl_not_test_pnl() -> None:
    ...
    # Construimos un escenario donde el segundo tendría mejor test,
    # pero el primero conserva el mejor train. El selector debe elegir
    # el primero porque test no participa en la selección.
    train_winner = replace(first, train_pnl=100.0, test_pnl=-999.0)
    test_winner = replace(second, train_pnl=50.0, test_pnl=999.0)
    altered = run.__class__(
        results=(train_winner, test_winner, *run.results[2:]),
        finding=run.finding.__class__(
            experiments=len(run.results),
...
def test_final_validation_rejects_incomplete_test_metrics() -> None:
    ...
    altered = run.__class__(
        results=tuple(incomplete if item.config == target.config else item for item in run.results),
        finding=run.finding,
        evidence=run.evidence,
        manifest=run.manifest,
        experiment_id=run.experiment_id,
