...
def test_kill_switch_does_not_close_or_modify_existing_virtual_position() -> None:
    switch = KillSwitch()
    engine = PaperTradingEngine(PaperPortfolio(10_000), kill_switch=switch, quantity=1)

    assert engine.process(pd.Series({"close": 100.0, "atr": 5.0, "signal": Signal.BUY})) == "OPEN BUY"

    switch.trigger("emergencia de simulación")
    result = engine.process(pd.Series({"close": 94.0, "signal": Signal.WAIT}))
...
def test_reset_allows_simulation_to_resume_explicitly() -> None:
    ...
    assert "STOPPED:" in engine.process(pd.Series({"close": 100.0, "atr": 1.0, "signal": Signal.BUY}))

    switch.reset()

    assert engine.process(pd.Series({"close": 100.0, "atr": 1.0, "signal": Signal.BUY})) == "OPEN BUY"
