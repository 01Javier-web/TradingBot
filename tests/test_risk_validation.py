...
def test_non_finite_stop_loss_distance_is_rejected() -> None:
    result = validate_risk_inputs(
        balance=10_000.0,
        risk_amount=50.0,
        daily_loss=20.0,
        open_positions=0,
        stop_loss_distance=nan,
    )
...
def test_non_positive_risk_amount_is_rejected() -> None:
    result = validate_risk_inputs(
        balance=10_000.0,
        risk_amount=0.0,
        daily_loss=20.0,
    ...
    assert result.valid is False
    assert any("risk_amount" in issue for issue in result.issues)
